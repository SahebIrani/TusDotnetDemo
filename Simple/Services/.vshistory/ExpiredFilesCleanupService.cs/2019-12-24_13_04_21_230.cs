...
namespace Simple.Services.vshistory.ExpiredFilesCleanupService.cs
{
	public class ExpiredFilesCleanupService : IHostedService, IDisposable
	{
		...
		public async Task StartAsync(CancellationToken cancellationToken)
		{
			...
			_timer = new Timer(async (e) =>
				await RunCleanup((CancellationToken)e), cancellationToken, TimeSpan.Zero, _expiration.Timeout);
		}
		...
		private async Task RunCleanup(CancellationToken cancellationToken)
		{
			try
			{
				...
				_logger.LogInformation($"Removed {numberOfRemovedFiles} expired files. " +
					$"Scheduled to run again in {_expiration.Timeout.TotalMilliseconds} ms");
			}
			catch (Exception exc)
			{
